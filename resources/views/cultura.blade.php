    ...
    <x-slot name="header">
        <div class="breadcrumb">
            ...
            <a class="text-xl font-semibold leading-tight text-gray-800"
                href="/cultura">{{ __('Cultura') }}</a>
        </div>
    </x-slot>
    <div class="py-12"
        style="background-image: url('/img/fondo_principal.jpg'); background-position: center top; background-repeat: no-repeat; background-size: cover; height:900px">
                            ...
                            <div class="py-4 pl-4 pr-6 border-l-2 border-gray-300 flex flex-wrap flex-row ">
                                <div class="w-full md:w-1/2 pr-2">
                                    <div class="font-serif-egyptian text-white">
                                        ...
                                        <div class="bg-[#0e0e07b8] shadow rounded-lg p-4">{{ $event['description'] }}
                                        </div>
                                    </div>
                                </div>
                                <div class="w-full md:w-1/2 pl-2">
                                    @if (isset($event['photo']))
                                        <div class="mt-4">
                                            <img src="{{ asset($event['photo']) }}" alt="{{ $event['title'] }}"
                                                class="max-w-full">
                                        </div>
                                    @endif
                                    ...
                                    @if (isset($event['video']))
                                        <div class="mt-4">
                                            <iframe width="90%" height="250"
                                                src="{{ htmlspecialchars($event['video']) }}" frameborder="0"
