        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="p-4 mb-5 py-5">
                <div class="overflow-x-auto">
                    ...
                    <form method="POST" action="{{ route('evaluacion_curiosidad') }}">
                        @csrf
                        @foreach ($questions as $question)
                            <!-- Campo de texto -->
                            <div class="mb-4">
                                <label for="nombre" class="block mb-2">{{ $question->p_pregunta }}:</label>
                                @if (isset($question->respuestas))
                                    @foreach ($question->respuestas as $respuesta)
                                        @if ($respuesta->r_state == 1)
                                            <input type="radio" name="{{ $respuesta->id }}" value="{{ $respuesta->id }}" required>
